...
router.post('/', authAdmin, async (req, res) => {
  try {
    ...
    // Enviar email a todos los usuarios avisando inscripciones abiertas
    try {
      const { rows: usuarios } = await pool.query("SELECT email, nombre FROM usuarios WHERE activo=true AND email NOT LIKE '%@test.com'");
      const fecha_str = a.fecha ? new Date(a.fecha).toLocaleDateString('es-AR',{weekday:'long',day:'numeric',month:'long'}) : '';
      const precio_str = a.precio_inscripcion > 0 ? '$'+Number(a.precio_inscripcion).toLocaleString('es-AR') : 'Gratis';
      const cats_str = catsCreadas.map(c => '<li style="margin-bottom:4px">'+c.nombre+' — '+( c.genero==='masculino'?'Masculino':c.genero==='femenino'?'Femenino':'Mixto')+' ('+c.cupo_max+' parejas)</li>').join('');
      const htmlEmail = '<div style="font-family:sans-serif;background:#07000f;color:#fff;padding:32px;border-radius:16px;max-width:520px;margin:0 auto"><div style="font-size:28px;font-weight:900;margin-bottom:4px">🤝 PÁDEL AMERICANO</div><div style="font-size:16px;color:#22c55e;font-weight:700;margin-bottom:20px">'+a.nombre+'</div><div style="background:rgba(34,197,94,.08);border:1px solid rgba(34,197,94,.2);border-radius:12px;padding:16px;margin-bottom:20px"><div style="font-size:13px;color:rgba(255,255,255,.6);margin-bottom:8px">📅 '+fecha_str+'</div><div style="font-size:13px;color:rgba(255,255,255,.6);margin-bottom:8px">🕘 '+((a.hora_inicio||'09:00').substring(0,5))+'hs</div><div style="font-size:13px;color:#22c55e;font-weight:700">💰 '+precio_str+'</div>'+( a.sede?'<div style="font-size:13px;color:rgba(255,255,255,.5);margin-top:8px">📍 '+a.sede+'</div>':'')+'</div>'+( catsCreadas.length?'<div style="margin-bottom:20px"><div style="font-size:12px;color:rgba(255,255,255,.4);margin-bottom:8px">CATEGORÍAS</div><ul style="margin:0;padding-left:16px;color:rgba(255,255,255,.8);font-size:13px">'+cats_str+'</ul></div>':'')+'<div style="background:rgba(255,255,255,.04);border-radius:12px;padding:16px;margin-bottom:20px"><div style="font-size:13px;font-weight:700;color:#22c55e;margin-bottom:12px">📱 ¿Cómo inscribirme?</div><ol style="margin:0;padding-left:16px;color:rgba(255,255,255,.75);font-size:13px;line-height:2"><li>Ingresá a <a href="https://cordobalux.com/padel" style="color:#22c55e">cordobalux.com/padel</a></li><li>Presioná <strong>JUGAR</strong> en la pantalla principal</li><li>Tocá el botón <strong>🏆 COMPETIR</strong></li><li>Seleccioná el tab <strong>🤝 AMERICANO</strong></li><li>Entrá al americano y tocá <strong>+ Inscribirme con mi pareja</strong></li><li>Buscá a tu compañero/a por nombre o email — debe estar registrado/a en la app</li><li>Confirmá con el botón <strong>INSCRIBIRME</strong></li></ol></div>'+( a.descripcion?'<div style="font-size:13px;color:rgba(255,255,255,.6);margin-bottom:20px;padding:12px;background:rgba(255,255,255,.03);border-radius:10px">'+a.descripcion+'</div>':'')+'<a href="https://cordobalux.com/padel" style="display:inline-block;background:linear-gradient(135deg,#16a34a,#22c55e);color:#fff;padding:14px 28px;border-radius:50px;text-decoration:none;font-weight:700;font-size:15px">¡Quiero inscribirme! →</a><div style="margin-top:20px;font-size:11px;color:rgba(255,255,255,.25);text-align:center">PadelConnect · cordobalux.com</div></div>';
