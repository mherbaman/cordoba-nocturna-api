...
# 1. Boton en sidebar
viejo_btn = 'onclick="navTo(\'reportes\',this)"><span class="icon">📊</span> Reportes</button>'
nuevo_btn = viejo_btn + '\n      <button class="nav-item" onclick="navTo(\'partidos-pub\',this)"><span class="icon">⚡</span> Partidos</button>'
html = html.replace(viejo_btn, nuevo_btn)
print("1 OK" if viejo_btn in open(ARCHIVO).read() else "1 SKIP")

# 2. Hook en navTo
viejo_nav = "if (page === 'promo-principal') cargarPromoPrincipal();\n}"
nuevo_nav = "if (page === 'promo-principal') cargarPromoPrincipal();\n  if (page === 'partidos-pub') cargarPartidosPubAdmin();\n}"
html = html.replace(viejo_nav, nuevo_nav)

# 3. Seccion HTML antes de <!-- MODAL -->
...
"""
html = html.replace('<!-- MODAL -->', SECCION + '<!-- MODAL -->', 1)
...
for i,l in enumerate(lines):
    if 'page-partidos-pub' in l or 'crearPartidoPublico' in l or '<!-- MODAL' in l:
